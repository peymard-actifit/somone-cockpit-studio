...
Tu dois retourner un JSON structuré avec:
{
  ...
  "conclusion": "Texte de conclusion"
    ...
    // GET /tutorial - Récupérer le tutoriel
    if (path === '/tutorial' && method === 'GET') {
      try {
        const tutorialData = await redis.get('somone_tutorial');
        
        if (tutorialData) {
          return res.json({ tutorial: tutorialData });
        } else {
          // Retourner un tutoriel vide si non initialisé
          return res.json({ tutorial: null });
        }
      } catch (error: any) {
        console.error('[Tutorial] Erreur GET:', error);
        return res.status(500).json({ error: 'Erreur serveur' });
      }
    }
    
    // POST /tutorial - Sauvegarder le tutoriel (admin uniquement)
    if (path === '/tutorial' && method === 'POST') {
      const authHeader = req.headers.authorization;
      if (!authHeader) {
        return res.status(401).json({ error: 'Token manquant' });
      }
      ...
      try {
        ...
        if (!tutorial) {
          return res.status(400).json({ error: 'Données du tutoriel manquantes' });
        }
        
        // Mettre à jour la date et la version
        tutorial.updatedAt = new Date().toISOString();
        tutorial.version = (tutorial.version || 0) + 1;
        tutorial.createdBy = decoded.id;
        
        // Sauvegarder dans Redis
        await redis.set('somone_tutorial', tutorial);
        
        console.log(`[Tutorial] Sauvegardé v${tutorial.version} par ${decoded.id}`);
        
        return res.json({ success: true, tutorial });
      } catch (error: any) {
        console.error('[Tutorial] Erreur POST:', error);
        return res.status(500).json({ error: 'Erreur serveur' });
      }
    }
    
    // POST /tutorial/translate - Traduire un texte en anglais via DeepL
    if (path === '/tutorial/translate' && method === 'POST') {
      const authHeader = req.headers.authorization;
      if (!authHeader) {
        return res.status(401).json({ error: 'Token manquant' });
      }
      ...
      try {
        ...
        if (!response.ok) {
          const errorText = await response.text();
          console.error('[Tutorial] Erreur DeepL:', errorText);
          return res.status(response.status).json({ error: 'Erreur traduction DeepL' });
        }
